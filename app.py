...
# ===============================
# 구글드라이브 업로드 (안정성 강화 버전)
# ===============================
def upload_to_drive(local_path, folder_id):
    """
    구글 드라이브 업로드 실패 시, 명확한 원인을 파악하여 에러를 발생시키는 안정화 버전
    """
    try:
        print(f"🚀 Google Drive 업로드를 시작합니다: {os.path.basename(local_path)}")
        ...
        media = MediaFileUpload(local_path, mimetype="text/csv") # 파일 타입에 맞게 "application/json" 또는 "text/csv"
        ...
        # 구글 API 에러 발생 시, 상세 내용을 파악하여 정확한 해결책을 제시합니다.
        error_details = error.content.decode('utf-8')
        print(f"❌ 구글 드라이브 업로드 실패! 원인: {error_details}")
        if 'storageQuotaExceeded' in error_details:
            # 이것이 바로 우리가 겪었던 핵심 문제입니다.
            raise Exception("구글 드라이브 권한 오류: 서비스 계정은 개인 저장 공간이 없습니다. 파일을 저장할 구글 드라이브 폴더의 '공유' 설정에서 서비스 계정 이메일 주소를 추가하고 '편집자' 권한을 부여했는지 반드시 확인해주세요.")
        elif 'File not found' in error_details:
             raise Exception(f"구글 드라이브 폴더 찾기 오류: GitHub Secrets에 등록된 폴더 ID({folder_id})가 유효하지 않습니다. 폴더가 삭제되었거나 ID가 잘못되었을 수 있습니다.")
        else:
            raise Exception(f"구글 드라이브 API 에러가 발생했습니다: {error_details}")
    except Exception as e:
        # 그 외 예기치 못한 에러
        raise e
