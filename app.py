#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# OliveYoung(국내) 랭킹 수집 + GDrive 업로드 + Slack 알림 (ScrapingAnt 422 완벽 대응 버전)
...
# GitHub Actions 환경변수 SCRAPER_API_KEY에 입력된 ScrapingAnt 키를 그대로 바인딩
...
# [★수정 핵심★] ScrapingAnt 422 대응 통짜 인코딩 패턴 교정 적용
def try_scrapingant_fetch(url="https://www.oliveyoung.co.kr/store/main/getBestList.do"):
    ...
    try:
        ...
        # Requests 내부 urlencode에만 의존하지 않고, ScrapingAnt가 오인하지 않도록 명시적으로 인코딩
        encoded_target_url = quote_plus(url)
        full_api_url = f"https://api.scrapingant.com/v2/general?api_key={SCRAPINGANT_API_KEY}&url={encoded_target_url}&browser=true"
        
        # 프록시 웹 브라우저 로딩 시간을 감안하여 타임아웃을 60초로 넉넉히 배정
        r = requests.get(full_api_url, timeout=60)
